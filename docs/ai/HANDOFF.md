...
- **V2 P1 — Local RAG infra** 🟡 IN PROGRESS
  - Local Ollama embeddings + Qdrant vector store. Core chunk/embed/upsert/retrieve code is implemented and under review remediation. Retrieval falls back to the existing full-corpus prompt whenever the corpus is tiny or either service is unavailable, so chat remains usable while infra is offline.
