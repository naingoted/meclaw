...
import { type IngestDocumentResult, ingesterFor } from "@meclaw/rag";
...
export async function enqueueSingle(db: Db, documentId: string, actorIp: string) {
  ...
  await logAudit(db, {
    ...
    entityType: "job",
    entityId: row.id,
    summary: `queued ingest for ${documentId}`,
    actorIp,
  });
  scheduleDrain(db);
  return row;
...
export async function runNextJob(db: Db, opts: { ingestFn?: IngestFn } = {}): Promise<boolean> {
  ...
  try {
    ...
    await db
      .update(ingestionJobs)
      .set({ status: "succeeded", chunksWritten: result.chunks, finishedAt: now })
      .where(eq(ingestionJobs.id, job.id))
      .execute();
    await db
      .update(documents)
      .set({ status: "ready", lastIngestedAt: now })
      .where(eq(documents.id, doc.id))
      .execute();
    await logAudit(db, {
      action: "ingest.succeed",
      entityType: "job",
      entityId: job.id,
      summary: `ingested "${doc.title}" (${result.chunks} chunks)`,
    });
  } catch (e) {
    ...
    await db
      ...
      .execute();
    if (doc)
      await db.update(documents).set({ status: "error" }).where(eq(documents.id, doc.id)).execute();
    await logAudit(db, {
      action: "ingest.fail",
      entityType: "job",
      entityId: job.id,
