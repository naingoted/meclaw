...
export async function createDocument(db: Db, input: DocumentInput, actorIp: string): Promise<DocumentRow> {
  const now = new Date();
  const row = {
...
export async function updateDocument(db: Db, id: string, input: DocumentInput, actorIp: string): Promise<DocumentRow> {
