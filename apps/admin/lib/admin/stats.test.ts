...
describe("computeStats", () => {
  ...
  it("lastIngest is the finishedAt of the most-recently-created succeeded job", async () => {
    ...
    await db.insert(ingestionJobs).values(job({
      status: "succeeded", createdAt: new Date("2026-01-01T00:00:00.000Z"), finishedAt: new Date("2026-01-01T00:00:00.000Z"),
    })).execute();
    await db.insert(ingestionJobs).values(job({
      status: "succeeded", createdAt: latest, finishedAt: latest,
    })).execute();
    // A later-created FAILED job must not be picked as the last ingest.
    await db.insert(ingestionJobs).values(job({
      status: "failed", createdAt: new Date("2026-03-01T00:00:00.000Z"), finishedAt: new Date("2026-03-01T00:00:00.000Z"),
    })).execute();
    const stats = await computeStats(db);
