...
describe("settings API", () => {
  ...
  it("GET returns settings", async () => { expect((await GET()).status).toBe(200); });
  it("PUT 400 on invalid body", async () => {
    const res = await PUT(new Request("http://x", { method: "PUT", body: JSON.stringify({ ...value, rag: { ...value.rag, topK: -5 } }) }));
