    ...
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="agents">
          <div className={STACK}>
            <Field label="Shared persona" help={HELP.persona} htmlFor="shared.persona" error={errAt("shared.persona")}>
            ...
            {agentKeys.map((key) => (
              <div key={key} className={`${STACK} border-t pt-5`}>
                ...
                {MODEL_LABELS[key] ? (
                  <Field label={MODEL_LABELS[key]} help={HELP.model} htmlFor={`agents.${key}.model`} error={errAt(`agents.${key}.model`)}>
                    <Input id={`agents.${key}.model`} {...register(p(`agents.${key}.model`))} />
                  </Field>
                ) : null}
                <Field label="Prompt" help={HELP.prompt} htmlFor={`agents.${key}.prompt`} error={errAt(`agents.${key}.prompt`)}>
                  <Textarea id={`agents.${key}.prompt`} {...register(p(`agents.${key}.prompt`))} />
                </Field>
                {key === "triage" ? (
                  <Field label="Routing confidence" help={HELP.confidence} htmlFor="agents.triage.confidence" error={errAt("agents.triage.confidence")}>
        ...
        <TabsContent value="rag">
          <div className={STACK}>
            ...
            <Field label="Score floor" help={HELP.scoreFloor} htmlFor="rag.scoreFloor" error={errAt("rag.scoreFloor")}>
              <Input id="rag.scoreFloor" type="number" step="0.01" {...register("rag.scoreFloor", { valueAsNumber: true })} />
            </Field>
            <Field label="Score threshold" help={HELP.scoreThreshold} htmlFor="rag.scoreThreshold" error={errAt("rag.scoreThreshold")}>
              <Input id="rag.scoreThreshold" type="number" step="0.01" {...register("rag.scoreThreshold", { valueAsNumber: true })} />
            </Field>
            <Field label="Cluster radius" help={HELP.clusterRadius} htmlFor="rag.clusterRadius" error={errAt("rag.clusterRadius")}>
              <Input id="rag.clusterRadius" type="number" step="0.01" {...register("rag.clusterRadius", { valueAsNumber: true })} />
            </Field>
            <Field label="Tiny-corpus threshold" help={HELP.tinyCorpusThreshold} htmlFor="rag.tinyCorpusThreshold" error={errAt("rag.tinyCorpusThreshold")}>
        ...
        <TabsContent value="public">
          <div className={STACK}>
            <Field label="Greeting" help={HELP.greeting} htmlFor="public.greeting" error={errAt("public.greeting")}>
              <Input id="public.greeting" {...register("public.greeting")} />
            </Field>
            <Field label="Suggestions" help={HELP.suggestions} htmlFor="public.suggestions" error={errAt("public.suggestions")}>
              <Controller
                control={control}
            ...
            <Field label="Cal.com URL" help={HELP.calUrl} htmlFor="public.calUrl" error={errAt("public.calUrl")}>
              <Input id="public.calUrl" {...register("public.calUrl")} />
            </Field>
            <Field label="GitHub URL" help={HELP.githubUrl} htmlFor="public.githubUrl" error={errAt("public.githubUrl")}>
              <Input id="public.githubUrl" {...register("public.githubUrl")} />
            </Field>
            <Field label="Contact email" help={HELP.contactEmail} htmlFor="public.contactEmail" error={errAt("public.contactEmail")}>
