"use client";
import { Table, THead, TBody, TR, TH, TD, Skeleton, EmptyState, relativeTime } from "@meclaw/ui";
  ...
  return (
    <div>
      ...
      {rows === null ? (
        ...
        <EmptyState title="No audit entries yet" hint="Admin actions (saves, ingests, deletes) will appear here." />
      ) : (
        <Table>
          <THead><TR><TH>Time</TH><TH>Action</TH><TH>Summary</TH></TR></THead>
          <TBody>
            {rows.map((a) => (
              <TR key={a.id}>
                <TD className="font-mono text-xs text-muted-foreground" title={new Date(a.ts).toLocaleString()}>{relativeTime(a.ts)}</TD>
                <TD>{a.action}</TD>
                <TD>{a.summary}</TD>
              </TR>
            ))}
