...
type ClusterSummary = {
  ...
  updatedAt: string; reasons: Record<string, number>;
};
type Miss = { id: string; query: string; reason: string; topScore: number | null; conversationId: string; createdAt: string };
type Detail = { cluster: { id: string; exemplarQuery: string | null; count: number; status: string }; misses: Miss[] };
...
export function GapsClient() {
  ...
  if (selectedId) {
    return <GapDetail id={selectedId} onBack={() => { setSelectedId(null); void load(); }} />;
  }
  ...
  return (
    <div>
      <PageHeader
        title="Gaps"
        subtitle={clusters ? `${clusters.length} ${status} cluster${clusters.length === 1 ? "" : "s"}` : undefined}
        action={
      ...
      {clusters === null ? (
        ...
        <EmptyState title="No gaps here" hint="Questions the corpus couldn't answer will cluster here as visitors ask them." />
      ) : (
        <Table>
          <THead><TR><TH>Question</TH><TH>Hits</TH><TH>Reasons</TH><TH>Status</TH><TH>Last seen</TH></TR></THead>
          <TBody>
            {clusters.map((c) => (
              <TR key={c.id}>
                <TD>
                ...
                <TD>{c.count}</TD>
                <TD className="font-mono text-xs text-muted-foreground">{reasonSummary(c.reasons)}</TD>
...
function GapDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  React.useEffect(() => {
    void (async () => { const d = await (await fetch(`/api/admin/gaps/${id}`)).json(); setDetail(d); })();
  }, [id]);

  async function ignore() {
    setBusy(true);
    try {
      await fetch(`/api/admin/gaps/${id}`, { method: "PATCH", body: JSON.stringify({ action: "ignore" }) });
      onBack();
  ...
  async function answer(title: string, body: string) {
    ...
    try {
      ...
      await fetch(`/api/admin/gaps/${id}`, { method: "PATCH", body: JSON.stringify({ action: "resolve", documentId: doc.id }) });
      onBack();
    } finally { setBusy(false); }
  }
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <Button variant="ghost-danger" loading={busy} onClick={ignore}>Ignore</Button>
            <Button loading={busy} onClick={() => setAnswering(true)}>Answer this gap</Button>
          </div>
        }
      />
      <Table>
        <THead><TR><TH>Question</TH><TH>Reason</TH><TH>Top score</TH><TH>When</TH></TR></THead>
        <TBody>
          {detail.misses.map((m) => (
            <TR key={m.id}>
              <TD>{m.query}</TD>
              <TD><StatusPill status={m.reason} /></TD>
              <TD className="font-mono text-xs">{m.topScore == null ? "—" : m.topScore.toFixed(2)}</TD>
...
function AnswerForm({
  defaultTitle, busy, onSubmit, onCancel,
}: { defaultTitle: string; busy: boolean; onSubmit: (title: string, body: string) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Write a knowledge document that answers this question. Saving creates the document, queues ingest, and marks the gap resolved.
      </p>
      <Input className="mb-2" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} />
      <Textarea className="mb-2 min-h-[40vh] font-sans" placeholder="Answer content (markdown)…" value={body} onChange={(e) => setBody(e.target.value)} />
      <div className="flex gap-2">
        <Button loading={busy} disabled={!title.trim() || !body.trim()} onClick={() => onSubmit(title, body)}>
          Save, ingest &amp; resolve
        </Button>
