...
export function DocumentsClient() {
  ...
  const loadJobs = React.useCallback(
    async () => setJobs(await (await fetch("/api/admin/jobs")).json()),
    [],
  );
  React.useEffect(() => {
    void (async () => {
  ...
  async function openEditor(id: string) {
    setOpeningId(id);
    try {
      setEditing(await (await fetch(`/api/admin/documents/${id}`)).json());
    } finally {
      setOpeningId(null);
    }
  }
              ...
              return (
                <TR key={d.id}>
                  <TD>
                    ...
                    >
                      {openingId === d.id ? (
                        <Spinner className="h-3.5 w-3.5 border-current border-t-transparent" />
                      ) : null}
                      {d.title}
                    </button>
                  ...
                  <TD>
                    {isDirty(d) ? (
                      <span className="text-accent" title="needs ingest">
                        ●
                      </span>
                    ) : (
...
function Editor({
  ...
  doc: Doc;
  onSave: (d: { id?: string; title: string; body: string; category?: string }) => Promise<void>;
  onCancel: () => void;
}) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await onSave({ id: doc.id || undefined, title, body, category: category || undefined });
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      />
      <Textarea
        className="mb-2 min-h-[50vh] font-sans"
        value={body}
