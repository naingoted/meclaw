"use client";
import { Button, StatTile, StatusPill, Skeleton, EmptyState, relativeTime, deriveStatusCounts } from "@meclaw/ui";
import * as React from "react";

type Job = { id: string; kind: string; status: string; error: string | null; chunksWritten: number | null; documentId: string | null; createdAt?: string | null };
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-4 gap-3">
        <StatTile label="Dirty" value={counts.dirty} tone={counts.dirty > 0 ? "warning" : "neutral"} />
        <StatTile label="Queued" value={counts.queued} />
        <StatTile label="Running" value={counts.running} tone={counts.running > 0 ? "running" : "neutral"} />
      ...
      {jobs === null ? (
        ...
        <EmptyState title="No ingest jobs yet" hint="Ingest a document, or reingest dirty docs to populate the log." />
      ) : (
        <div className="overflow-hidden rounded-md border border-border">
          <div className="grid grid-cols-[100px_1fr_70px_60px] gap-3 border-b border-border bg-card px-3 py-2 text-[10px] uppercase tracking-wider text-muted-foreground">
            <span>Status</span><span>Document</span><span>Chunks</span><span></span>
          </div>
          {jobs.map((j) => (
            <div key={j.id} className="grid grid-cols-[100px_1fr_70px_60px] items-center gap-3 border-b border-border px-3 py-2 text-sm last:border-0">
              <StatusPill status={j.status} />
              <div className="min-w-0">
                <div className="truncate text-foreground">{j.documentId ?? j.kind}</div>
                {j.status === "failed" && j.error ? (
                  <div className="mt-0.5 truncate text-xs text-destructive" title={j.error}>{j.error}</div>
                ) : null}
              </div>
              <span className="font-mono text-xs text-muted-foreground">{j.chunksWritten ?? "—"}</span>
              <span className="text-right">
                {j.status === "failed" ? (
                  <Button size="sm" variant="outline" loading={retrying === j.id} onClick={() => retry(j.id)}>Retry</Button>
                ) : null}
              </span>
            </div>
          ))}
