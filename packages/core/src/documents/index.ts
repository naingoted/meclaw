...
import { documents, gapClusters } from "../db/schema";
...
/** Mark a document successfully ingested so isDirty() flips false until the next edit. */
export async function markIngested(db: Db, id: string): Promise<void> {
  await db
    .update(documents)
    .set({ status: "ready", lastIngestedAt: new Date() })
